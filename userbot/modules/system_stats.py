...
ALIVE_MESAJ = ["Yüce Noxus'a Selam Olsun 😈", "Tüm Botlara Selam Çalışmaya Devam 🤖"]
...
async def pipcheck(pip):
    ...
    if pipmodule:
        ...
        await pip.edit("`Bir örnek görmek için .nox pip komutunu kullanın.`")


@register(outgoing=True, pattern="^.alive$")
async def amialive(e):
    if not e.text[0].isalpha() and e.text[0] not in ("/", "#", "@", "!"):
        await e.edit(f"`{random.choice(ALIVE_MESAJ)} Nox Çalışıyor")
...
CMD_HELP.update({
    "alive": ".alive\
    \nKullanım: Nox Userbotunun çalışıp çalışmadığını kontrol etmek için kullanılır."
})

...
async def randomise(items):
    ...
    if len(itemo) < 2:
        await items.edit(
            "`2 veya daha fazla eşya gerekli. Daha fazla bilgi için .nox random komutunu gir.`"
        )
        return
...
async def shutdown(event):
    ...
    await event.edit("`Ben Gidiyeeeeh*`")
    if BOTLOG:
...
async def restart(event):
    await event.edit("`Yeniden başlıyorum ehehhehe...`")
    if BOTLOG:
...
async def bot_support(wannahelp):
    ...
    await wannahelp.edit("[Buradan](http://t.me/NoxDestek) destek grubumuza ulaşabilirsiniz.")


@register(outgoing=True, pattern="^.creator$")
async def creator(e):
    await e.edit("Bu bot \n"
                 "[Noxus](https://t.me/bynoxus) tarafından geliştirilmiştir. \n")
...
async def repo_is_here(wannasee):
    ...
    await wannasee.edit("[Nox Repo](https://github.com/bysenseix/noxuserbot)")

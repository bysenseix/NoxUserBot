...
UPSTREAM_REPO_URL = os.environ.get(
    "UPSTREAM_REPO_URL",
    "https://github.com/bysenseix/NoxUserBot")
    ...
    try:
        ...
        async def handler(event):
            if not event.message.from_id == uid:
                await event.reply(f'`Merhaba ben` @NoxUserBot`! Ben sahibime (`@{me.username}`) yardımcı olmak için varım, yaani sana yardımcı olamam :/ Ama sen de bir Nox açabilirsin; Kanala bak` @NoxUserBot')
            else:
        ...
        async def inline_handler(event):
            ...
            result = None
            query = event.text
            if event.query.user_id == uid and query == "@NoxUserBot":
                rev_text = query[::-1]
                buttons = paginate_help(0, moduller, "helpme")
                result = builder.article(
                    f"Lütfen Sadece .yardım Komutu İle Kullanın",
                    text="{}\nYüklenen Modül Sayısı: {}".format(
                        "Merhaba! Ben @NoxUserBot kullanıyorum!\n\nhttps://github.com/bysenseix/NoxUserBot", len(moduller)),
                    buttons=buttons,
                    link_preview=False
                )
                ...
                result = builder.article(
                    "© @NoxUserBot",
                    text=f"@NoxUserBot ile güçlendirildi",
                    buttons=[],
                    link_preview=True
                )
                ...
                result = builder.article(
                    "© @NoxUserBot",
                    text="""@NoxUserBot'u kullanmayı deneyin!
Hesabınızı bot'a çevirebilirsiniz ve bunları kullanabilirsiniz. Unutmayın, siz başkasının botunu yönetemezsiniz! Alttaki GitHub adresinden tüm kurulum detayları anlatılmıştır.""",
                    buttons=[
                        [custom.Button.url("Kanala Katıl", "https://t.me/NoxUserBot"), custom.Button.url(
                            "Gruba Katıl", "https://t.me/NoxDestek")],
                        [custom.Button.url(
                            "GitHub", "https://github.com/bysenseix/NoxUserBot")]
                    ],
                    link_preview=False
                )
        ...
        async def on_plug_in_callback_query_handler(event):
            if event.query.user_id == uid:  # pylint:disable=E0602
                ...
                reply_pop_up_alert = "Lütfen kendine bir @NoxUserBot aç, benim mesajlarımı düzenlemeye çalışma!"
            ...
            if event.query.user_id == uid:  # pylint:disable=E0602
                ...
                if len(cmdhel) > 90:
                    help_string = str(CMD_HELP[modul_name])[
                        :90] + "\n\nDevamı için .nox " + modul_name + " yazın."
                else:
                    help_string = str(CMD_HELP[modul_name])

...
async def ban(bon):
    ...
    if user.id in BRAIN_CHECKER:
        await bon.edit(
            "`Ban Hatası! Nox Yetkilisini yasaklayamam.`"
        )
        return
...
async def spider(spdr):
    ...
    if user.id in BRAIN_CHECKER:
        await spdr.edit(
            "`Mute Hatası! Nox Yetkilisini susturamam.`"
        )
        return
...
async def gspider(gspdr):
    ...
    # Eğer kullanıcı sudo ise
    if user.id in BRAIN_CHECKER:
        await gspdr.edit("`Gmute Hatası! Nox Yetkilisini küresel olarak susturamam.`")
        return
...
async def kick(usr):
    ...
    if user.id in BRAIN_CHECKER:
        await usr.edit(
            "`Kick Hatası! Nox Yetkilisini gruptan tekmeleyemem.`"
        )
        return
